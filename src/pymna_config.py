...
TEXTS = {
    "jp": {
        "guide_title": "📌 入力ガイド",
        "guide_text": "[基本フォーマット]\n種別 名前 接続点+ 接続点- 値 [許容差/範囲]\n\n* 抵抗 (R), 電圧源 (V), 電流源 (I):\n  R R1 1 0 1k 1  (1kΩ ±1%)\n  R R2 1 0 1k 0.9k/1.1k  (1kΩ, 最小0.9kΩ, 最大1.1kΩ - 非対称設定可)\n  V V1 1 0 12V 5  (12V ±5%)\n  I I1 1 0 1mA  (1mA, 固定)\n\n* 電圧制御電圧源 (E - オペアンプ等):\n  E 名前 出力+ 出力- 制御+ 制御- 利得\n  例: E OP1 3 0 1 2 100k  (出力 V(3)-V(0) = 100k * (V(1)-V(2)))\n\n* 電流制御電流源 (F):\n  F 名前 出力+ 出力- 制御電圧源名 利得 [許容差/範囲]\n  例: F F1 3 0 V_sense 2 1.8/2.2  (出力電流 I(3->0) = 利得 * I(V_sense))\n  (V_sense はネットリスト内に存在する電圧源である必要があります)\n\n[対応単位]\nk (kilo), meg (mega), m (milli), u (micro), n (nano), p (pico)\n\n[電力解析タグ (抵抗のみ)]\n行末にコメントとして追加します:\n; @P=0.1W @T_S=70 @T_M=155\n  @P: 定格電力 [W]\n  @T_S: 軽減開始温度 [°C] (例: 標準70, 車載125)\n  @T_M: 最高使用温度 [°C] (例: 155)\n\n[Excelからのコピー＆ペーストに対応]",
